...
function MyRoutes() {
    ...
    const handleDelete = async (routeId, routeSeq) => {
        const confirmDelete = window.confirm('경로를 삭제하시겠습니까?');

        if (confirmDelete) {
            try {
                await axios.delete(`http://localhost:8080/api/route-delete/${routeId}/${routeSeq}`);
                window.location.reload();
            } catch (error) {
                console.error('경로 삭제 중 오류가 발생했습니다.', error);
            }
        }
    };
                ...
                <ul>
                    {routes.map((route, index) => (
                        <li key={index}>
                            ...
                            <button onClick={() => handleDelete(route.userid, route.route_seq)}>
                                경로삭제
                            </button>
                        </li>
                    ))}
